...
describe("Multi-Device Bug Hunting", () => {
  ...
  describe("Device restart during multi-device operation", () => {
    /**
     * One device restarts while another device is actively messaging.
     */
    it("should handle one device restarting while sibling sends messages", async () => {
      await runControlledScenario({
        steps: [
          { type: "addDevice", actor: "alice", deviceId: "alice-1" },
          { type: "addDevice", actor: "alice", deviceId: "alice-2" },
          ...
          // alice-1 continues chatting
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
            message: "while-sibling-offline-1",
            waitOn: "auto",
          },
          {
            type: "send",
            ...
            to: "alice",
            message: "bob-reply-1",
            waitOn: { actor: "alice", deviceId: "alice-1" },
          },
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
            message: "while-sibling-offline-2",
            waitOn: "auto",
          },
          // alice-2 comes back online
          { type: "restart", actor: "alice", deviceId: "alice-2" },
          // alice-2 should receive bob's reply that was sent while offline
          { type: "expect", actor: "alice", deviceId: "alice-2", message: "bob-reply-1" },
          // And alice-2 should be able to send
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-2" },
            to: "bob",
          ...
          { type: "expect", actor: "bob", deviceId: "bob-1", message: "back-online" },
        ],
      })
    })
  })
