...
describe("Edge Cases (Controlled Relay)", () => {
  ...
  describe("Device synchronization edge cases", () => {
    ...
    it("should handle message sent while one device is offline", async () => {
      await runControlledScenario({
        steps: [
          { type: "addDevice", actor: "alice", deviceId: "alice-1" },
          { type: "addDevice", actor: "bob", deviceId: "bob-1" },
          ...
          // Bob's device-2 goes offline
          { type: "close", actor: "bob", deviceId: "bob-2" },
          // Alice sends messages while device-2 is offline
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
            message: "while-offline-1",
            waitOn: { actor: "bob", deviceId: "bob-1" },
          },
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
            message: "while-offline-2",
            waitOn: { actor: "bob", deviceId: "bob-1" },
          },
          // Device-1 received them
          { type: "expect", actor: "bob", deviceId: "bob-1", message: "while-offline-1" },
          { type: "expect", actor: "bob", deviceId: "bob-1", message: "while-offline-2" },
          // Device-2 comes back online
          { type: "restart", actor: "bob", deviceId: "bob-2" },
          // Device-2 should receive the messages
          { type: "expect", actor: "bob", deviceId: "bob-2", message: "while-offline-1" },
          { type: "expect", actor: "bob", deviceId: "bob-2", message: "while-offline-2" },
        ],
      })
    })
  })

  describe("Restart and recovery scenarios", () => {
    /**
     * Test that sessions survive restarts and messages sent during
     * restart are properly delivered.
     */
    /**
     * NOTE: Messages sent while recipient is completely offline may not be
     * automatically replayed on restart, depending on relay behavior.
     * This test verifies that at minimum, NEW messages after restart work,
     * and that we can manually trigger replay of stored events.
     */
    it("should recover session after restart and continue communication", async () => {
      await runControlledScenario({
        steps: [
          { type: "addDevice", actor: "alice", deviceId: "alice-1" },
          { type: "addDevice", actor: "bob", deviceId: "bob-1" },
          ...
          // Bob is restarting...
          { type: "close", actor: "bob", deviceId: "bob-1" },
          // Bob comes back
          { type: "restart", actor: "bob", deviceId: "bob-1" },
          // Session should continue working after restart
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
          ...
          { type: "expect", actor: "bob", deviceId: "bob-1", message: "after-restart" },
          {
            type: "send",
            from: { actor: "bob", deviceId: "bob-1" },
            to: "alice",
          ...
          { type: "expect", actor: "alice", deviceId: "alice-1", message: "back-online" },
        ],
      })
    })

    it("should handle both parties restarting simultaneously", async () => {
      await runControlledScenario({
        steps: [
          { type: "addDevice", actor: "alice", deviceId: "alice-1" },
          { type: "addDevice", actor: "bob", deviceId: "bob-1" },
          // Establish session with some message history
          {
            type: "send",
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
          ...
          // Both restart at the same time
          { type: "close", actor: "alice", deviceId: "alice-1" },
          { type: "close", actor: "bob", deviceId: "bob-1" },
          { type: "restart", actor: "alice", deviceId: "alice-1" },
          { type: "restart", actor: "bob", deviceId: "bob-1" },
          // Communication should resume
          {
            ...
            from: { actor: "alice", deviceId: "alice-1" },
            to: "bob",
            message: "after-both-restart",
            waitOn: "auto",
          },
          { type: "expect", actor: "bob", deviceId: "bob-1", message: "after-both-restart" },
          {
            type: "send",
            from: { actor: "bob", deviceId: "bob-1" },
            to: "alice",
          ...
          { type: "expect", actor: "alice", deviceId: "alice-1", message: "confirmed" },
        ],
      })
    })
  })
