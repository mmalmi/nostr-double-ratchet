...
export function encodeCompactDeviceLinkRequest(request: {
  ...
  if (!requestSecret || !isHex32Bytes(request.deviceAppKeyPubkey) || !isHex32Bytes(requestSecret)) {
    throw new Error("Invalid compact device link request")
  }
  return `${request.deviceAppKeyPubkey.toLowerCase()}.${requestSecret.toLowerCase()}`
}

export function parseCompactDeviceLinkRequest(input: string): DeviceLinkRequest | null {
  const parts = input.trim().split(".")
  if (parts.length !== 2) return null
  const [deviceAppKeyPubkey, requestSecret] = parts
  if (!isHex32Bytes(deviceAppKeyPubkey) || !isHex32Bytes(requestSecret)) return null
...
export function deterministicLinkInviteForDeviceLinkRequest(
  ...
  if (!isHex32Bytes(request.deviceAppKeyPubkey) || !isHex32Bytes(request.requestSecret)) {
    throw new Error("Invalid compact device link request")
  }
