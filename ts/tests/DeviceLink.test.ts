...
describe("DeviceLink", () => {
  it("encodes scan-to-approve requests with device labels in compact metadata", () => {
    ...
    expect(local.code).toMatch(/^[0-9a-f]{64}\.[0-9a-f]{64}\.[A-Za-z0-9_-]+$/);
    ...
    expect(local.request.deviceAppKeyPubkey).toBe(
      getPublicKey(deviceAppKeySecretKey),
    );
    ...
    expect(local.request.deviceLabel).toBe("Safari on macOS");
    expect(local.request.clientLabel).toBe("Iris Chat Web");

    const parts = local.code.split(".");
    expect(parts).toHaveLength(3);
    expect(parts[0]).toBe(local.request.deviceAppKeyPubkey);
    expect(parts[1]).toBe(toHex(requestSecretKey));
    ...
    expect(parsed?.requestedAt).toBe(41);
    expect(parsed?.deviceLabel).toBe("Safari on macOS");
    expect(parsed?.clientLabel).toBe("Iris Chat Web");
  });
  ...
  it("rejects malformed approval codes", () => {
    ...
    expect(
      parseCompactDeviceLinkRequest(`${"a".repeat(64)}.${"b".repeat(64)}`),
    ).toBeNull();
    expect(
      parseCompactDeviceLinkRequest(
        `${"a".repeat(64)}.${"b".repeat(64)}.not-base64!*`,
      ),
    ).toBeNull();
  });
  ...
  it("derives deterministic NDR link invites from the compact secret", () => {
    ...
    expect(toHex(invite.inviterEphemeralPrivateKey!)).toMatch(
      /^be3f1cca6354c294/,
    );
    expect(invite.inviterEphemeralPublicKey).toBe(
      repeated.inviterEphemeralPublicKey,
    );
