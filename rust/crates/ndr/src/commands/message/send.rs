...
use nostr_double_ratchet::{
    ...
    SessionManager, SessionManagerEvent, StorageAdapter, CHAT_MESSAGE_KIND, EXPIRATION_TAG,
    MESSAGE_EVENT_KIND,
};
...
#[cfg(test)]
use nostr_double_ratchet::ManagedSession as Session;
...
pub(super) async fn send_message_impl(
    ...
    let event_ids = emit_manager_output(
        session_manager_store.as_ref(),
        &manager_tx,
        manager.send_event(recipient_pk, unsigned)?,
    )?;
...
pub async fn receipt(
    ...
    let _event_ids = emit_manager_output(
        session_manager_store.as_ref(),
        &manager_tx,
...
pub async fn typing(
    ...
    let _event_ids = emit_manager_output(
        session_manager_store.as_ref(),
        &manager_tx,
        manager.send_typing(recipient_pk, None)?,
    )?;
