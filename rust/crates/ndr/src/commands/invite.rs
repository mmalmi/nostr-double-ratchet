...
pub async fn listen(
    ...
    let invites = storage.list_invites()?;
    if invites.is_empty() {
        anyhow::bail!("No pending invites. Create an invite first with 'ndr invite create'.");
    }
    ...
    // Subscribe to invite response events addressed to us
    ...
    while let Ok(notification) = notifications.recv().await {
        if let RelayPoolNotification::Event { event, .. } = notification {
            // Try to match with each pending invite
            for stored_invite in storage.list_invites()? {
                ...
                match invite.process_invite_response(&event, our_private_key.clone()) {
                    Ok(Some((session, their_pubkey, _device_id))) => {
                        ...
                        output.event("invite.accepted", InviteAcceptedEvent {
