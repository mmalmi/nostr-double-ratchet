...
async fn test_invite_publish_creates_event() {
    ...
    let publish = run_ndr(data_dir, &["invite", "publish", "--device-id", "test-device", "-l", "test"]).await;
    ...
    assert!(invite_event.tags.iter().any(|t| t.first().map(|s| s.as_str()) == Some("ephemeralKey")));
    assert!(invite_event.tags.iter().any(|t| t.first().map(|s| s.as_str()) == Some("sharedSecret")));
