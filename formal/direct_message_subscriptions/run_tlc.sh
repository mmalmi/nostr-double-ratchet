...
if [[ "${MODE}" == "all" ]]; then
  if run_cfg DirectMessageSubscriptions.current.cfg; then
    echo "Expected DirectMessageSubscriptions.current.cfg to fail, but it passed." >&2
    exit 1
  else
    echo "DirectMessageSubscriptions.current.cfg failed as expected."
  fi
fi
