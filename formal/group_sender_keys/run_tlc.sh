...
usage() {
  cat <<'USAGE'
Usage:
  ./formal/group_sender_keys/run_tlc.sh [--mode all|ci]
...
if [[ "${MODE}" == "all" ]]; then
  if run_cfg GroupSenderKeys.current.cfg; then
    echo "Expected GroupSenderKeys.current.cfg to fail, but it passed." >&2
    exit 1
  else
